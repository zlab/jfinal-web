...
(function (factory) {
    ...
    var Tree = function (element, options) {
        ...
        this.$element.on('click.fu.tree', '.tree-item', $.proxy(function (ev) {
            if ($(ev.target).is('.icon-item.fa-check') || $(ev.target).is('.icon-item.fa-times')) {
                this.checkItem(ev.target);
            } else {
    ...
    Tree.prototype = {
                ...
                $.each(items.data, function (index, value) {
                    ...
                    if (value.type === 'folder') {
                        ...
                        if ('additionalParameters' in value) {
                            ...
                            if ('item-checked' in value.additionalParameters
                                && value.additionalParameters['item-checked'] == true) {

                                setTimeout(function () {
                                    $entity.find('.icon-item').trigger('click');
                                }, 0);
                            }

                            if ('item-selected' in value.additionalParameters
                                && value.additionalParameters['item-selected'] == true) {
        ...
        selectItem: function (el) {
            ...
            if (this.options.multiSelect) {
                ...
                //
                $all.removeClass('tree-selected');//.find('i').removeClass(this.options['selected-icon'])
                    //.addClass(this.options['unselected-icon']);//ACE
                data.push(selData);
            }
        ...
        checkedItems: function () {
            var $chk = this.$element.find('.icon-item.fa-check');
            var data = [];

            $.each($chk, function (index, value) {
                data.push($(value).parentsUntil('.tree-item').parent().data());
            });
            return data;
        },
